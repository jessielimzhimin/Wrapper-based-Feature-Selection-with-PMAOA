...
DL = 10; %parameter of mutation-restarting phase (historical interval, t)
...
G = 0;               % Generation counter (used to index the Leader archive)
gALS = 0;            % Dedicated ALS alternation counter (HDL/PRI), incremented
                     % exactly ONCE per generation so mod(gALS,2) cleanly
                     % alternates HDL (odd) and PRI (even) every generation.
fes2 = 1;
...
while fes <= MaxFE
    ...
    G = G + 1; % Update generation counter (Leader archive index)
    Leader(G,:) = BestSol.Position;

    gALS = gALS + 1; % Update ALS alternation counter ONCE per generation

    %% Adaptive Learning Stage (HDL / PRR alternation)
    newsol = repmat(empty_individual,nPop,1);

    for i = 1:nPop

        if mod(gALS,2)~=0 % gALS is odd -> Historical Difference Learning (HDL)
            ...
            % if generation counter G is larger than the historical
            % interval (DL), calculate new solution based on the
            % leader-drift vector and the peer-difference vector
            if G > DL
        ...
        else % gALS is even -> Partial Random Reinitialization (PRR)
